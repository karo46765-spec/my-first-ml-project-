...
raw_data = {
    ...
    'Employement_type' : ['Salaried','Unemployed','Salaried','Salareid','Self-Employed','Unemployed','Salareid','Self-Employed']
}
...
print(f"Original Row Count : {len(df)}->Balanced Row count after SMOTE:{len(x_reworked)}")
print(f"Class 1 (Risk) instances before: {nm.sum(y == 1)} -> After SMOTE : {nm.sum(y_reworked == 1)}")
